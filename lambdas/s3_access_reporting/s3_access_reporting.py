#!/usr/bin/python3
# s3_access_reporting.py

"""S3 Access Reporting Module.

This module takes an S3 bucket name as input and, with the right permissions in place,
uses Athena to process CloudTrail data and create S3 access reports for new
...
def conditionally_create_database(
    ...
    print("Conditionally created an Athena database for S3 access reporting.")
    return response
...
def get_existing_report_sundays(bucket: str, account: str) -> Set[str]:
    ...
    client = boto3.client("s3")
    prefix = f"reports/aws-s3-object-access/{account}"
    report_sundays = set()
    for page in client.get_paginator("list_objects_v2").paginate(
...
def generate_report(
    ...
    query = fr"SELECT useridentity.principalId as Requester, eventname as Request, json_extract(requestparameters, '$.bucketName') as BucketName, json_extract(requestparameters, '$.key') as Object, sourceipaddress as SourceIP, count(*) as NumRequests FROM {database}.{table} WHERE eventsource = 's3.amazonaws.com' AND region in ("
    ...
    query += fr") GROUP BY useridentity.principalId, eventname, json_extract(requestparameters, '$.bucketName'), json_extract(requestparameters, '$.key') , sourceipaddress ORDER BY useridentity.principalId, eventname"
    report_output_location = (
        f"s3://{bucket}/reports/aws-s3-object-access/{account}/{sunday}/"
    )
    query_result_config = {
...
def handler(event: dict, context: dict):
    """Lambda handler for S3 access report generation.
    ...
    for sunday in missing_report_sundays:
        ...
        while success != True:
            try:
                generate_report(
                    athena_session, database, table, bucket, account, sunday, regions,
                )
            except:
